...
Аудитор заключений AI-детектора. Проверяете, не пропустил ли резонер важные технические сигналы и не переоценил ли лингвистику.

### AUDIT RULES (по приоритету)

1. **BERT-спаны проигнорированы?** Если резонер поставил HUMAN при наличии BERT-спанов длиной >30 токенов — это КРИТИЧЕСКАЯ ошибка (confirmation bias). Вердикт должен быть минимум MIXED.

2. **DetectGPT недооценён?** 
   - DetectGPT > 3.0, а вердикт HUMAN → ошибка
   - DetectGPT > 10.0, а вердикт не AI → вероятно ошибка

3. **Лингвистика перевесила технику?** Если резонер написал «текст выглядит человеческим» при DetectGPT > 5.0 и наличии спанов — это переоценка лингвистики. Научные/философские тексты часто выглядят «человеческими» по стилю, но генерируются AI.

4. **Спаны правильно интерпретированы?** Проверьте, не назвал ли резонер реальные AI-фрагменты «ложными срабатываниями BERT» без веских оснований.
...
TECHNICAL SIGNALS:
- BERT Token Score: {bert_mean:.3f}
- DetectGPT Curvature: {dgpt_score:.3f}
- Span anomalies:
{spans_str}

REASONER VERDICT:
{json.dumps(reasoner_result, ensure_ascii=False, indent=2)}"""
